...
export function AdminMembershipApprovalsPage() {
  ...
  return <AdminShell><div className="mx-auto max-w-6xl">
    ...
    {decision ? <Modal title={`${decision.approve ? "Approve" : "Reject"} ${decision.request.request_type}`} description="The decision reason and financial outcome become part of the permanent audit trail." onClose={() => { setDecision(null); setReason(""); }}><div className="ops-modal__body"><label>Decision reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} /></label><div className="ops-modal__actions"><Button variant="outline" onClick={() => { setDecision(null); setReason(""); }}>Cancel</Button><Button variant={decision.approve ? "primary" : "danger"} disabled={reason.trim().length < 10 || decide.isPending} onClick={() => decide.mutate(decision)}>Confirm decision</Button></div></div></Modal> : null}
  </div></AdminShell>;
}
