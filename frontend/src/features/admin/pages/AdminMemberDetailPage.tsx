...
export function AdminMemberDetailPage() {
  ...
  return <AdminShell><div className="mx-auto max-w-7xl">
    ...
    {decision ? <Modal title={`${decision.approve ? "Approve" : "Reject"} ${decision.request.request_type}`} description="This reason becomes part of the permanent audit record." onClose={() => { setDecision(null); setReason(""); }}><div className="ops-modal__body"><label>Decision reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} /></label><div className="ops-modal__actions"><Button variant="outline" onClick={() => setDecision(null)}>Cancel</Button><Button variant={decision.approve ? "primary" : "danger"} disabled={reason.trim().length < 10 || action.isPending} onClick={() => action.mutate({ path: decision.request.request_type === "freeze" ? `/admin/freeze-requests/${decision.request.id}/decision` : `/admin/cancellation-requests/${decision.request.id}/decision`, body: { approve: decision.approve, decision_reason: reason, outcome: decision.outcome } })}>Confirm decision</Button></div></div></Modal> : null}
    {revokeOpen ? <Modal title="Revoke membership access" description="QR generation and class access will be blocked immediately." onClose={() => { setRevokeOpen(false); setReason(""); }}><div className="ops-modal__body"><div className="rounded-xl bg-destructive/10 p-3 text-sm text-destructive"><ShieldAlert className="mb-2 size-5" /> Revocation is a sensitive action and cannot be silent.</div><label>Mandatory reason<textarea value={reason} onChange={(event) => setReason(event.target.value)} /></label><div className="ops-modal__actions"><Button variant="outline" onClick={() => setRevokeOpen(false)}>Cancel</Button><Button variant="danger" disabled={reason.trim().length < 10 || action.isPending} onClick={() => action.mutate({ path: `/admin/members/${id}/revoke`, body: { reason } })}>Revoke membership</Button></div></div></Modal> : null}
  </div></AdminShell>;
}
