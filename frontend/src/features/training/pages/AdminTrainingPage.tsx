...
import { MUSCLES, MUSCLE_LABELS, type Exercise, type ExercisePage, type Muscle } from "@/features/training/types";
...
export function AdminTrainingPage() {
  ...
  return <AdminShell><div className="mx-auto max-w-7xl">
    ...
    {editing ? <Modal title={editing === "new" ? "Add exercise" : `Edit ${editing.name}`} description="Usage and safety text appear in the member guide." error={save.isError ? toUiError(save.error).message : undefined} onClose={() => setEditing(null)}><form className="ops-modal__body" onSubmit={form.handleSubmit((values) => save.mutate({ id: editing === "new" ? undefined : editing.id, values }))}><div className="grid gap-4">
      ...
      {(["primary_muscles", "secondary_muscles"] as const).map((key) => <fieldset key={key} className="rounded-lg border border-border p-3"><legend className="px-1 text-sm font-bold">{key === "primary_muscles" ? "Primary muscles trained" : "Secondary muscles trained"}</legend><p className="mb-2 text-xs text-muted-foreground">Select one or more muscles. The circular selector shows the selected role without oversized checkbox boxes.</p><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{MUSCLES.map((muscle) => <label key={muscle} className="flex min-h-8 cursor-pointer items-center gap-2 rounded-md border border-border px-2 py-1 text-xs font-semibold transition hover:border-primary has-[:checked]:border-primary has-[:checked]:bg-primary/10"><input className="peer sr-only" type="checkbox" value={muscle} aria-label={`${MUSCLE_LABELS[muscle]} ${key === "primary_muscles" ? "primary" : "secondary"} muscle`} {...form.register(key)} /><span aria-hidden className="grid size-3.5 shrink-0 place-items-center rounded-full border border-muted-foreground peer-checked:border-primary peer-checked:bg-primary"><span className="size-1.5 rounded-full bg-primary-foreground opacity-0 peer-checked:opacity-100" /></span><span>{MUSCLE_LABELS[muscle]}</span></label>)}</div>{form.formState.errors[key] ? <p className="text-xs text-destructive">{form.formState.errors[key]?.message}</p> : null}</fieldset>)}
