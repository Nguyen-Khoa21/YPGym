...
export function NotificationsPage() {
  ...
  const readAll = useMutation({ mutationFn: () => apiRequest("/notifications/me/read-all", { method: "POST", token }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["notifications"] }) });
  const readOne = useMutation({ mutationFn: (id: string) => apiRequest(`/notifications/me/${id}/read`, { method: "POST", token }), onSuccess: () => queryClient.invalidateQueries({ queryKey: ["notifications"] }) });
