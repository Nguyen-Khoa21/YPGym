...
## September 24 post-Day-60 Feature 7

Feature 7 adds no schema migration. Member-owned history, day detail, exercise comparisons and the weekly muscle map are calculated from existing attendance plus authoritative workout set/name/muscle snapshots. The documented weighted-set-exposure metric counts each set as 1.0 per primary and 0.5 per secondary muscle, including zero-load/bodyweight sets; exact reps and loads remain separate. Focused isolated PostgreSQL verification passed five workout logging/history tests, covering account isolation, pagination, attended-only days, empty and cross-year weeks, bodyweight, snapshot stability, comparisons and calendar/map parity. The final isolated suite passed **119 tests with seven existing Starlette deprecation warnings in 61.62 seconds**. Web lint/build and mobile typecheck/lint, nine Node tests and Android export passed. Normal dependency health and Alembic current/check passed at `20260922_0011`; responsive authenticated web and Expo-web QA rendered the empty calendar/week, map/list, metric disclosure and exercise-history state. Physical Android/iOS/phone rendering and populated-history client QA were not exercised.
