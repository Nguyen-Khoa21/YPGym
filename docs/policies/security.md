...
- Development registration/reset and welcome messages use the ignored `backend/storage/mail/new/` Maildir outbox, configurable with `DEVELOPMENT_MAIL_DIR`. Private links are not logged; PostgreSQL stores only their token hashes. Protect and exclude the outbox from archives. Test environments skip development authentication messages. Feature 4 adds configurable SMTP transport and a deduplicated, retryable PostgreSQL welcome-email intent; external delivery is unverified until the owner privately configures and tests an actual sender. See `docs/api/email-delivery.md`.
...
Day55 implementation and local verification are complete. September 17 found generated invoices in the old local ZIP; it is superseded. `scripts/package_release.py` packages Git-managed/non-ignored source, rejects runtime storage, secret environment/key/database/cache paths and links escaping the repository, and emits per-file hashes plus a ZIP integrity inventory. Three regression tests pass. See `docs/release/archive-inventory.md` for the replacement package. This exclusion/pattern review does not prove an exhaustive secret audit. Deployed proxy-path verification remains outside the local scope; configured SMTP settings alone do not prove real delivery.
