...
async def main():
    ...
    async with AsyncSessionLocal() as session:
        ...
        session.add_all([UserMembership(user_id=user.id, plan_id=plan.id, start_date=date.today(), expiry_date=date.today() + timedelta(days=30), status="active") for user in users])
