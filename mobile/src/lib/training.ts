...
export type WorkoutToday = {
  ...
  eligible: boolean;
  reason: string | null;
  session: null | { id: string; workout_date: string; attendance_session_id: string; exercises: { id: string; exercise_id: string; name: string; primary_muscles: Muscle[]; secondary_muscles: Muscle[]; created_at: string; sets: { id: string; set_order: number; reps: number; weight: string; unit: 'kg' | 'lb' }[] }[] };
};
