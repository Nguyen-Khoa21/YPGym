...
export const muscleLabels: Record<Muscle, string> = { chest: 'Chest', back: 'Back', front_delts: 'Front delts', lateral_delts: 'Lateral delts', rear_delts: 'Rear delts', rhomboids: 'Rhomboids', traps: 'Traps', shoulders: 'Shoulders (legacy)', biceps: 'Biceps', triceps: 'Triceps', forearms: 'Forearms', core: 'Core', quadriceps: 'Quadriceps', hamstrings: 'Hamstrings', glutes: 'Glutes', calves: 'Calves' };
...
export type WorkoutSession = { id: string; workout_date: string; attendance_session_id: string; exercises: { id: string; exercise_id: string; name: string; primary_muscles: Muscle[]; secondary_muscles: Muscle[]; created_at: string; sets: WorkoutSet[] }[] };
...
export type ExerciseHistoryPage = { gym_timezone: string; items: { workout_date: string; workout_exercise_id: string; name: string; sets: WorkoutSet[]; set_count: number; total_reps: number; max_external_load_kg: string; comparison_to_previous: null | { sets: 'more' | 'same' | 'less'; reps: 'more' | 'same' | 'less'; external_load: 'more' | 'same' | 'less' } }[]; page: { page: number; page_size: number; total: number; pages: number } };
