...
export type Notification = { id: string; title: string; message: string; read_at: string | null; created_at: string; category: string };
export type NotificationPage = { items: Notification[]; unread_count: number; page: { page: number; pages: number; total: number } };
export type Preferences = { email_enabled: boolean; in_app_enabled: boolean; expiry_reminders_enabled: boolean; broadcasts_enabled: boolean };
