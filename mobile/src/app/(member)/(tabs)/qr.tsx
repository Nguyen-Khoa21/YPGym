...
export default function QrScreen() {
  ...
  useEffect(() => {
    const subscription = AppState.addEventListener('change', (state) => {
      ...
      if (state === 'active' && eligible) void refetchQr().then((result) => setForegroundReady(AppState.currentState === 'active' && result.isSuccess));
    });
  ...
  return <Screen onRefresh={() => { void access.refetch(); if (eligible) void qr.refetch(); }} refreshing={access.isRefetching || qr.isRefetching}>
    ...
    {eligible ? <Card accent><Text style={[textStyles.muted, { textAlign: 'center' }]}>Scan this code at the gym entrance</Text><View style={{ alignItems: 'center' }}><Pill label={showCode ? 'Live entry code' : 'Checking code'} /></View>
      {qr.isLoading ? <Busy label="Issuing your rotating pass" /> : null}
      {qr.isError ? <Message title="Code unavailable" detail={errorMessage(qr.error)} action="Try again" onAction={() => void qr.refetch()} /> : null}
      {showCode && qr.data ? <View style={{ alignItems: 'center', paddingVertical: 18, gap: 17 }}><View style={{ backgroundColor: '#ffffff', padding: 16, borderRadius: 15 }}><QRCode value={qr.data.token} size={Math.min(width - 102, 252)} backgroundColor="#ffffff" color="#171717" /></View><Text style={textStyles.lime}>Refreshes in {seconds}s</Text></View> : qr.data && !qr.isError ? <Message title="Pass refreshing" detail="A fresh server-issued code is required before check-in." action="Refresh now" onAction={() => void qr.refetch()} /> : null}
