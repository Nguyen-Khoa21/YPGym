...
export default function ClassesScreen() {
  ...
  return <Screen refreshing={classes.isRefetching} onRefresh={() => void classes.refetch()}><Brand /><Heading title="Find a Class" detail="Live places, trainers and your booking state." />
    ...
    <View style={{ flexDirection: 'row', gap: 8, marginVertical: 17 }}>{(['all', 'today', 'tomorrow'] as const).map((value) => <Pressable key={value} accessibilityRole="button" accessibilityState={{ selected: day === value }} onPress={() => setDay(value)} style={{ backgroundColor: day === value ? colors.lime : colors.card, borderRadius: 10, paddingHorizontal: 17, paddingVertical: 11 }}><Text style={{ color: day === value ? colors.black : colors.text, fontWeight: '700', textTransform: 'capitalize' }}>{value}</Text></Pressable>)}</View>
    ...
    {classes.isError ? <Message title="Classes unavailable" detail={errorMessage(classes.error)} action="Retry" onAction={() => void classes.refetch()} /> : null}
    {classes.data && !classes.data.booking_eligible ? <Message title="Booking unavailable" detail={classes.data.eligibility_reason ?? 'An active membership is required.'} action="View plans" onAction={() => router.push('/(member)/renew')} /> : null}
    ...
    <View style={{ marginTop: 20 }}>{visible.map((item) => <Card key={item.id} accent={item.member_state === 'booked'}>
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', gap: 10 }}><Text style={textStyles.lime}>{formatDate(item.start_at)} · {formatTime(item.start_at)}</Text><Text style={{ color: colors.text, fontWeight: '800' }}>{item.remaining_capacity} left</Text></View>
