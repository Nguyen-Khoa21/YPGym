...
export default function InvoicesScreen() {
  ...
  return <Screen refreshing={invoices.isRefetching} onRefresh={() => void invoices.refetch()}><PageTop title="Invoices" fallback="/(member)/(tabs)/profile" /><Heading title="Payment records" detail="Confirmed simulated payments and immutable invoice details." />
    {invoices.isLoading ? <Busy label="Loading invoices" /> : null}{invoices.isError ? <Message title="Invoices unavailable" detail={errorMessage(invoices.error)} action="Retry" onAction={() => void invoices.refetch()} tone="error" /> : null}
    {invoices.data?.length === 0 ? <Message title="No invoices yet" detail="A successful membership purchase will appear here." /> : null}
    {invoices.data?.map((item) => <Card key={item.id}><Text style={textStyles.subheading}>{item.plan_name}</Text><Text style={textStyles.accent}>{formatMoney(item.amount)}</Text><Text style={textStyles.muted}>Invoice {item.invoice_number} · {formatDate(item.transaction_date)}</Text><Text style={textStyles.muted}>Coverage expires {formatDate(item.membership_expiry_date)}</Text></Card>)}
  </Screen>;
}
