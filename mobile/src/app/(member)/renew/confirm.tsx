...
export default function ConfirmRenewalScreen() {
  ...
  return <Screen><PageTop title="Review Membership" fallback="/(member)/renew" /><Heading title="One last check" detail="YPGym uses a simulated payment for this project. No real payment method is charged." />
    ...
    <View style={{ minHeight: 68, flexDirection: 'row', alignItems: 'center', gap: 12, marginVertical: 18, backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1, borderRadius: 18, paddingHorizontal: 16 }}><Switch accessibilityLabel="Confirm simulated payment" value={confirmed} onValueChange={setConfirmed} trackColor={{ false: colors.surfaceStrong, true: colors.positive }} thumbColor={colors.white} /><Text style={[textStyles.muted, { flex: 1 }]}>I confirm this simulated payment and membership purchase.</Text></View>
    {purchase.isError ? <Message title="Payment not completed" detail={errorMessage(purchase.error)} action="Retry" onAction={() => purchase.mutate()} tone="error" /> : null}
