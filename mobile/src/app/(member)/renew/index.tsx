...
export default function RenewScreen() {
  ...
  return <Screen><PageTop title="Renew Membership" onBack={() => router.back()} /><Heading title="Current Status" />
    ...
    {dashboard.data ? <Card accent><Text style={textStyles.eyebrow}>{dashboard.data.membership ? 'Active plan' : 'No current plan'}</Text><Text style={textStyles.subheading}>{dashboard.data.membership?.plan_name ?? 'Choose your membership'}</Text>{dashboard.data.membership ? <><Text style={{ color: colors.lime, fontSize: 39, fontWeight: '900' }}>{dashboard.data.membership.days_remaining} <Text style={textStyles.muted}>days remaining</Text></Text><Pill label={dashboard.data.membership.status} /></> : <Text style={textStyles.muted}>Your plan will begin when the backend confirms the simulated payment.</Text>}</Card> : null}
