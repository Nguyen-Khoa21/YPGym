...
export default function RenewScreen() {
  ...
  return <Screen><PageTop title="Renew Membership" fallback="/(member)/(tabs)/dashboard" /><Heading title="Current Status" />
    ...
    {dashboard.data ? <Card accent><Text style={textStyles.eyebrow}>{dashboard.data.membership ? `${dashboard.data.membership.status} plan` : 'No current plan'}</Text><Text style={textStyles.subheading}>{dashboard.data.membership?.plan_name ?? 'Choose your membership'}</Text>{dashboard.data.membership ? <><View style={{ flexDirection: 'row', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}><Text style={{ color: colors.lime, fontSize: 39, lineHeight: 48, fontWeight: '900' }}>{dashboard.data.membership.days_remaining}</Text><Text style={textStyles.muted}>days remaining</Text></View><Pill label={dashboard.data.membership.status} tone={dashboard.data.qr_access.eligible ? 'lime' : 'coral'} /><Text style={textStyles.muted}>{dashboard.data.membership.message}</Text></> : <Text style={textStyles.muted}>Your plan will begin when the backend confirms the simulated payment.</Text>}</Card> : null}
