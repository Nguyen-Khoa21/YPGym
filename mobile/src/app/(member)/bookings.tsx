...
export default function BookingsScreen() {
  ...
  const targetId = Array.isArray(params.bookingId) ? params.bookingId[0] : params.bookingId;
  ...
  const orderedBookings = bookings.data?.bookings.slice().sort((left, right) => Number(right.id === targetId) - Number(left.id === targetId));
  return <Screen refreshing={bookings.isRefetching} onRefresh={() => void bookings.refetch()}><PageTop title="My Bookings" fallback="/(member)/(tabs)/classes" /><Heading title="Your class queue" detail={bookings.data ? `Cancellations close ${bookings.data.cancellation_window_hours} hours before class start.` : undefined} />
    {targetId ? <Message title="Opened from your class reminder" detail="The matching booking is shown first." tone="success" /> : null}
    {bookings.isLoading ? <Busy label="Loading your bookings" /> : null}{bookings.isError ? <Message title="Bookings unavailable" detail={errorMessage(bookings.error)} action="Retry" onAction={() => void bookings.refetch()} tone="error" /> : null}
    ...
    {orderedBookings?.length ? <><Text style={[textStyles.subheading, { marginBottom: 12 }]}>Bookings</Text>{orderedBookings.map((entry) => <Card key={entry.id} accent={entry.id === targetId}><Pill label={entry.status} tone={entry.status === 'booked' ? 'positive' : 'neutral'} /><Text style={textStyles.subheading}>{entry.gym_class.title}</Text><Text style={textStyles.muted}>{formatDate(entry.gym_class.start_at)} · {formatTime(entry.gym_class.start_at)} · {entry.gym_class.location}</Text>{entry.status === 'booked' && entry.can_cancel ? <Action label="Cancel booking" outline danger disabled={change.isPending} onPress={() => confirm(entry.id, entry.gym_class.title, false)} /> : null}</Card>)}</> : null}
    {bookings.data?.waitlists.length ? <View style={{ marginTop: 22 }}><Text style={[textStyles.subheading, { marginBottom: 12 }]}>Waitlists</Text>{bookings.data.waitlists.map((entry) => <Card key={entry.id}><Pill label={entry.status} tone={entry.status === 'waiting' ? 'warning' : 'neutral'} /><Text style={textStyles.subheading}>{entry.gym_class.title}</Text><Text style={textStyles.muted}>{formatDate(entry.gym_class.start_at)} · Position {entry.position}</Text>{entry.status === 'waiting' ? <Action label="Leave waitlist" outline disabled={change.isPending} onPress={() => confirm(entry.gym_class.id, entry.gym_class.title, true)} /> : null}</Card>)}</View> : null}
  </Screen>;
}
